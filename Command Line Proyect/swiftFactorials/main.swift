...
func line(){
    print("Factorial Calculator v0.1.0 - AOx0\n",
          "n!    :      Factorial\n",
          "n!!   :      Semi-Factorial\n",
    ...
    while true {
        ...
        if text.contains("(n!)") && text.contains("!)!"){
            let nums = getNums(text)
            nums != nil ? print(n(nums!,3)) : SyntaxError()
        } else if text.contains("n!!"){
            let nums = getNums(text)
            nums != nil ? print(n(nums!,2)) : SyntaxError()
        } else if text.contains("n!"){
            let nums = getNums(text)
            nums != nil ? print(n(nums!,1)) : SyntaxError()
...
func main(){
    var arg = CommandLine.arguments; arg.removeFirst()
    let text = arg.joined(separator: " ")
    if text.contains("nlll") {
        let nums = getNums(text)
        nums != nil ? print(n(nums!,3)) : SyntaxError()
    } else if text.contains("nll") {
        let nums = getNums(text)
        nums != nil ? print(n(nums!,2)) : SyntaxError()
    } else if text.contains("nl") {
        let nums = getNums(text)
        nums != nil ? print(n(nums!,1)) : SyntaxError()
    } else if text.contains("exit"){
        exit(0)
